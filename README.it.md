...
├── lang/                        # Directory localizzazione
│   ├── Language.php             # Sistema caricamento lingue
│   ├── it.php                   # Traduzioni italiano
...
**Funzionamento del Rate Limiting:**

Il sistema di rate limiting in `api.php` funziona così:

1. **Identificazione IP**: Ottiene l'indirizzo IP del client da `$_SERVER['REMOTE_ADDR']` e lo usa come identificatore unico
2. **Tracking per IP**: Crea un file JSON separato per ogni IP: `logs/ratelimit_{md5(IP)}.json`
3. **Configurazione**:
   - `FM_RATE_LIMIT`: massimo richieste per finestra (default: 30)
   - `FM_RATE_LIMIT_WINDOW`: durata finestra in secondi (default: 60)
4. **Logica**:
   - Legge il file JSON dell'IP se esiste
   - Se i dati sono più vecchi della finestra temporale → resetta contatore a 0
   - Se il contatore >= limite → restituisce HTTP 429 "Too many requests"
   - Altrimenti incrementa contatore e salva nel file
5. **Sicurezza**:
   - Basato su IP, non su sessione → non aggirabile cancellando cookie sessione
   - Ogni IP ha il proprio file → isolamento tra utenti
   - Reset automatico dopo scadenza finestra temporale

**Esempio:**
- Limite: 30 richieste/60 secondi
- L'IP 192.168.1.1 fa 25 richieste → tutte accettate
- La 31° richiesta entro 60 secondi → bloccata con 429
- Dopo 60 secondi → contatore reset automaticamente

## Localizzazione

Il file manager supporta multipli lingue con un sistema di localizzazione completo.
...
1. Crea un file nella directory `lang/` con il codice lingua (es. `ru.php`)
2. Copia la struttura da `it.php` o `en.php`
3. Traduci tutte le chiavi
4. Aggiungi il codice lingua a `FM_AVAILABLE_LANGUAGES` in `config.php`

### Cambio lingua

Gli utenti possono cambiare lingua tramite il selettore nell'header. La preferenza viene salvata in un cookie per 30 giorni.
